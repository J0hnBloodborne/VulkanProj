...
void transitionImageLayout(VkDevice device, VkCommandPool commandPool, VkQueue graphicsQueue, VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, VkPhysicalDevice physicalDevice)
{
    VkCommandBuffer commandBuffer = beginSingleTimeCommands(device, commandPool);
        ...
        if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
            ...
            destinationStage = VK_PIPELINE_STAGE_TRANSFER_BIT;        } else if (oldLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL && 
...
VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
    ...
    // If your dream format is unavailable, just take the first thing Vulkan offers.
...
VkPresentModeKHR chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
    ...
    // VK_PRESENT_MODE_FIFO_KHR always works, but it's just regular vsync.
...
bool checkValidationLayerSupport(const std::vector<const char*>& validationLayers) {
    ...
    for (const char* layerName : validationLayers) {
        ...
        if (!layerFound) {
            return false; // bro, you’re missing a layer
        }
    }
    return true; // you’re good, all layers found
}
